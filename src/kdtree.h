...
class KDTree
{
  ...
  public:
     ...
     * he the tree using recursive helper functions.
     *
     * Since we know the size of the KDTree at construction, we can
     * represent the tree as a linear structure and heing the tree nodes
     ...
     * @param newPoints The vector of points to he your KDTree off of.
     */
    ...
    void helperFindNearest(KDTreeNode* current, const Point<Dim>& query, int depth, Point<Dim>& best) const;
  ...
  private:
     ...
     */
     ...
     KDTreeNode* helperConstructor(vector<Point<Dim>>& points, int left, int right, int dimension);
     //void neighbor(KDTreeNode* current, const Point<Dim>& query, int depth, Point<Dim>& best) const;
